...
$(document).ready(function(){
    ...
    async function pathing(fromStr, toStr, fromNum, toNum) {// FROM (str), TO (str), FROM (number), TO (number)
        ...
        if (!Array.isArray(fromNum)) {fromNum = [parseInt(fromNum)];console.log('fromNum was not an array')} // to array
        if (!Array.isArray(toNum)) {toNum = [parseInt(toNum)];console.log('toNum was not an array')}
        ...
        paths.forEach(function callback(path, pathIndex) {
            ...
            respStr += "<p align=center><b>Путь №"+(pathIndex+1)+": из локации "+path.from+" в локацию "+path.to+"</b></p>";
            pathingLoop:
                while (!ended) {
